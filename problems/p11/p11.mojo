...
fn conv_1d_simple[
    ...
    # 1.handle cases where can handle entire window
    # 2.handle cases when win size is 2
    # 3.handle cases when win size is 1
    # this is very poor logic handling, anyways, I think its valid for now
    # if (global_i + (CONV - 1)) < SIZE:
    #     output[global_i] = (shared_a[local_i] * shared_b[0]) + (shared_a[local_i + 1] * shared_b[1]) + (shared_a[local_i + 2] * shared_b[2])
    # elif global_i == 4: 
    #     output[global_i] = (shared_a[local_i] * shared_b[0]) + (shared_a[local_i + 1] * shared_b[1])
    # elif global_i == 5:
...
fn conv_1d_block_boundary[
    ...
    ### load convolution memory to shared
    if local_i < CONV_2:
        shared_b[local_i] = b[local_i]

    barrier()

    # With the extra padding we can now, compute the sliding window
